...
jmmi_fil <- jmmi_2 %>%
  select(Year, Month, Week, Location, USD) %>%
  filter(Year == 2020 & Month == 9 & Week == 37) %>%
  arrange(USD)
...
m <- leaflet() %>%
        ...
        addCircleMarkers(data = jmmi_coord,
          fillColor = ~func(USD),
            ...
            label = ~Location,
            labelOptions = labelOptions(noHide = T, textOnly = TRUE, direction = "bottom", style=list("font-size"="14px", 
                                                                                                    "text-shadow"="10px 10px 10px '#ffffff'",
                                                                                                    "color"="rgb(88, 88, 91)",
                                                                                                    "font-family" = "Arial Narrow"
            )),
        ...
        addCircleMarkers(data = jmmi_coord_2,
          fillColor = ~func_2(USD),
            ...
            weight = 1,
            stroke = NULL,
            group = 'September First Week SSP Rate') %>%
        addLayersControl(baseGroups = c('September Third Week SSP Rate', 'September First Week SSP Rate'),
            options = layersControlOptions(collapsed = FALSE),
            ) %>%
        hideGroup('September First Week Median SSP Rate') %>%
        addLegend("bottomright", pal = func, values = jmmi_coord$USD,
            title = "Third Week SSP Parallel Market Rate",
            labFormat = labelFormat(prefix = "SSP "),
            opacity = 1,
            layerId = 'September Third Week Median SSP Rate',
            group = 'September Third Week SSP Rate',
            na.label = 'No USD Available')   %>%
        addLegend("bottomright", pal = func_2, values = jmmi_coord_2$USD,
            title = "First Week SSP Parallel Market Rate",
            labFormat = labelFormat(prefix = "SSP "),
            opacity = 1,
            layerId = 'September First Week SSP Rate',
            group = 'September First Week SSP Rate') %>%
